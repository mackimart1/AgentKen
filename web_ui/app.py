...
def index():
    ...
    agents = entities.get("result", {}).get("agents", []) if entities.get("status") == "success" else []
    tools = entities.get("result", {}).get("tools", []) if entities.get("status") == "success" else []
...
def create_agent_api():
    ...
    try:
        ...
        if result['status'] == 'success':
            log_audit_event(user_id, "create_agent", data['name'], {
                "description": data['description'],
                "capabilities": data['capabilities']
            })
...
def create_tool_api():
    ...
    try:
        ...
        if result['status'] == 'success':
            log_audit_event(user_id, "create_tool", data['name'], {
                "description": data['description'],
                "parameters": data['parameters'],
                "return_type": data['return_type']
            })
...
def list_entities_api():
    ...
    # Check permissions
    if entity_type in ['agents', 'all'] and not role_manager.check_permission(user_id, Permission.VIEW_AGENTS):
        return jsonify({"error": "Insufficient permissions"}), 403
    
    if entity_type in ['tools', 'all'] and not role_manager.check_permission(user_id, Permission.VIEW_TOOLS):
        return jsonify({"error": "Insufficient permissions"}), 403
...
def entities_page():
    ...
    if not (role_manager.check_permission(user_id, Permission.VIEW_AGENTS) or 
            role_manager.check_permission(user_id, Permission.VIEW_TOOLS)):
        return render_template('error.html', 
                             error="Insufficient permissions",
