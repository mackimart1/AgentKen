...
COLLECTION_NAME = "agentk_memories" # Used for ChromaDB context, less relevant for SQLite direct
...
DEFAULT_MEMORY_DECAY_RATE = 0.05  # Importance reduction per decay cycle for non-accessed memories
...
class MemoryConfig:
    ...
    db_batch_size: int = DEFAULT_DB_BATCH_SIZE # Currently conceptual, for future bulk ops
...
class ConnectionPool:
    ...
    def get_connection(self) -> sqlite3.Connection:
        try:
            ...
            logger.warning("Connection pool timeout. Trying to create a temporary connection.")
            conn = self._create_connection() # Fallback, potentially exceeding max_connections temporarily
...
class SemanticSearch:
    ...
    def __init__(self, model_name: str, batch_size: int):
        ...
        try:
            ...
            logger.error(f"Failed to load SentenceTransformer model '{model_name}': {e}. Semantic search will be disabled.")
            self.model = None
        self.embedding_cache = LRUCache(maxsize=1000) # Cache for individual text embeddings
    ...
    def get_embeddings(self, texts: List[str]) -> List[Optional[np.ndarray]]:
        ...
        if texts_to_encode:
            try:
                embeddings_generated = self.model.encode(texts_to_encode, batch_size=self.batch_size, show_progress_bar=False)
                with self.cache_lock:
...
class EnhancedMemoryManager:
    def __init__(self, config: Optional[MemoryConfig] = None):
        ...
        self.connection_pool = ConnectionPool(self.db_path, self.config.max_db_connections)
        if self.config.enable_semantic_search:
            self.semantic_search_handler = SemanticSearch(self.config.embedding_model_name, self.config.embedding_batch_size)
        else:
            self.semantic_search_handler = None
        
        self.executor = ThreadPoolExecutor(max_workers=os.cpu_count() or 1) # For background tasks and async ops
    ...
    def _compress_value(self, value: str) -> bytes:
        return zlib.compress(value.encode('utf-8')) if self.config.enable_compression else value.encode('utf--8')

    def _decompress_value(self, value_bytes: bytes) -> str:
        return zlib.decompress(value_bytes).decode('utf-8') if self.config.enable_compression else value_bytes.decode('utf-8')

    def _value_to_record(self, row: sqlite3.Row) -> MemoryRecord:
        return MemoryRecord(
            id=row['id'], key=row['key'], value=self._decompress_value(row['value']),
            memory_type=row['memory_type'], agent_name=row['agent_name'], task_info=row['task_info'],
            ...
            content_hash=row['content_hash'], metadata=json.loads(row['metadata']) if row['metadata'] else None,
            version=row['version']
        )
    ...
    def add_memory_async(self, key: str, value: str, memory_type: str = "general",
                     agent_name: Optional[str] = None, task_info: Optional[str] = None,
                     importance: float = 5.0, metadata: Optional[Dict[str, Any]] = None,
                     related_memories: Optional[List[Tuple[str, str, float]]] = None):
        """Asynchronously adds or updates a memory."""
        return self.executor.submit(self.add_memory, key, value, memory_type, agent_name,
                                    task_info, importance, metadata, related_memories)

    def add_memory(self, key: str, value: str, memory_type: str = "general",
                   agent_name: Optional[str] = None, task_info: Optional[str] = None,
                   importance: float = 5.0, metadata: Optional[Dict[str, Any]] = None,
                   related_memories: Optional[List[Tuple[str, str, float]]] = None) -> Optional[MemoryRecord]:
        ...
        try:
            with self.connection_pool.get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute("SELECT id, version, content_hash FROM memories WHERE key = ?", (key,))
                ...
                if existing: # Update existing memory
                    ...
                    if self.config.enable_versioning and existing['content_hash'] != content_hash:
                        new_version_number = current_version + 1
                        # Move old main record to versions table
                        cursor.execute("SELECT value, metadata FROM memories WHERE id = ?", (memory_id,))
                        ...
                        ''', (memory_id, old_main['value'], old_main['metadata'], current_version, memory_id))
                    ...
                    cursor.execute('''
                        ...
                        WHERE id=?
                    ''', (compressed_value, memory_type, agent_name, task_info, importance,
                          current_time.isoformat(), content_hash, embedding_blob, metadata_json,
                          new_version_number, memory_id))
                    ...
                    cursor.execute('''
                        ...
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    ''', (key, compressed_value, memory_type, agent_name, task_info, importance,
                          current_time.isoformat(), current_time.isoformat(), 1, content_hash,
                          embedding_blob, metadata_json, new_version_number))
                    memory_id = cursor.lastrowid
                    if self.config.enable_versioning: # Add first version explicitly
                         cursor.execute('''
                            INSERT INTO memory_versions (memory_id, value, metadata, version_number, timestamp)
                            VALUES (?, ?, ?, ?, ?)
                        ''', (memory_id, compressed_value, metadata_json, new_version_number, current_time.isoformat()))
                ...
                if new_row:
                    ...
                    logger.info(f"Memory '{key}' added/updated. Version: {record.version}. ID: {record.id}")
                    return record
                return None

        except sqlite3.Error as e:
            logger.error(f"SQLite error adding/updating memory '{key}': {e}", exc_info=True)
        except Exception as e:
            logger.error(f"Unexpected error adding/updating memory '{key}': {e}", exc_info=True)
        return None

    def _update_relationships(self, cursor: sqlite3.Cursor, source_id: int,
        ...
        for rel_key, rel_type, strength in related_memories:
            ...
            if target_row:
                ...
                logger.warning(f"Cannot create relationship: Related key '{rel_key}' not found.")
    ...
    def get_memory(self, key: str) -> Optional[MemoryRecord]:
        with self.cache_lock:
            if key in self.memory_cache:
                ...
                self.executor.submit(self._async_update_access_stats, cached_record.id or key, True)
        ...
        try:
            with self.connection_pool.get_connection() as conn:
                ...
                if row:
                    ...
                    self._async_update_access_stats(record.id, False) # Update DB access stats
    ...
    def _async_update_access_stats(self, memory_identifier: Union[int, str], is_id: bool):
        ...
        try:
            ...
            logger.error(f"Failed to async update access stats for {memory_identifier}: {e}")
    ...
    def semantic_search(self, query_text: str, limit: int = 5, min_similarity: float = 0.5) -> List[MemoryRecord]:
        ...
        try:
            ...
            for row in rows:
                ...
                if mem_embedding_blob:
                    ...
                    similarity = self.semantic_search_handler.compute_similarity(query_embedding, mem_embedding)
    ...
    def get_related_memories(self, key: str, relationship_type: Optional[str] = None,
                             min_strength: float = 0.1, limit: int = 10) -> List[MemoryRecord]:
        results = []
        try:
            with self.connection_pool.get_connection() as conn:
                cursor = conn.cursor()
                ...
                if not source_row:
                    logger.warning(f"Source key '{key}' not found for getting related memories.")
                    return []
            ...
            logger.error(f"Failed to get related memories for '{key}': {e}", exc_info=True)
        return results
    ...
    def get_memory_history(self, key: str) -> List[MemoryVersion]:
        ...
        try:
            ...
            logger.error(f"Failed to get memory history for '{key}': {e}", exc_info=True)
        return history

    def _update_memory_decay(self):
        logger.info("Running memory decay update...")
        decay_threshold_date = datetime.now() - timedelta(days=1) # Only decay if not accessed recently
        try:
            with self.connection_pool.get_connection() as conn:
                cursor = conn.cursor()
                ...
                ''', (self.config.min_importance, self.config.decay_rate,
                      decay_threshold_date.isoformat(), self.config.min_importance, self.config.critical_importance))
                conn.commit()
                logger.info(f"Memory decay applied. {cursor.rowcount} memories affected.")
        except Exception as e:
    ...
    def _cleanup_old_memories(self, max_age_days_override: Optional[int] = None):
        age_days = max_age_days_override if max_age_days_override is not None else self.config.max_age_days
        cutoff_date = datetime.now() - timedelta(days=age_days)
        logger.info(f"Running cleanup for memories older than {age_days} days (before {cutoff_date.isoformat()}) and below critical importance.")
        ...
        try:
            with self.connection_pool.get_connection() as conn:
                ...
                conn.commit()
                logger.info(f"Memory cleanup completed. {cursor.rowcount} memories deleted.")
    ...
    def get_memory_stats(self) -> Dict[str, Any]:
        stats = {"total_memories": 0, "total_versions": 0, "total_relationships": 0, "cache_size": 0}
        try:
            with self.connection_pool.get_connection() as conn:
                cursor = conn.cursor()
            ...
            while not self._maintenance_thread_stop_event.is_set():
                try:
                    ...
                    if hasattr(self, '_last_optimization_time'):
                        if time.time() - self._last_optimization_time > self.config.refresh_interval_seconds * 24: # ~Once a day
                    ...
                    logger.error(f"Error in maintenance loop submission: {e}", exc_info=True)
                
                # Wait for the next interval or until stop event is set
                self._maintenance_thread_stop_event.wait(self.config.refresh_interval_seconds)
            logger.info("Memory maintenance thread stopped.")

        self._maintenance_thread = threading.Thread(target=maintenance_loop, daemon=True, name="MemoryMaintenanceThread")
...
def get_global_memory_manager(config: Optional[MemoryConfig] = None) -> EnhancedMemoryManager:
...
def memory_manager_tool(task: str) -> Dict[str, Any]:
    ...
    write_match = re.match(r"write key '(.+?)' value '(.+?)'(?:\s+type '(.+?)')?(?:\s+importance (\d+\.?\d*))?(?:\s+meta '(.+?)')?", task_lower)
    if write_match:
        ...
        future = manager.add_memory_async(key, value, memory_type=mem_type, importance=importance, metadata=metadata)
        # For this tool, we might want to wait a short period or just confirm submission
        try:
            record = future.get(timeout=5.0) # Wait up to 5s for completion for immediate feedback
            if record:
                 return {"status": "success", "message": f"Memory '{key}' stored.", "key": key, "id": record.id, "version": record.version}
            else:
                 return {"status": "error", "message": f"Failed to store memory '{key}'.", "key": key}
        except TimeoutError:
            return {"status": "success", "message": "Memory storage submitted (processing in background).", "key": key}
        except Exception as e:
            logger.error(f"Error processing write for key '{key}': {e}")
            return {"status": "error", "message": f"Error storing memory '{key}': {str(e)}", "key": key}
    ...
    if delete_match:
        ...
        return {"status": "error", "message": f"Failed to delete key '{key}' (possibly not found)."}
    ...
    query_match = re.match(r"query '(.+?)'(?:\s+limit (\d+))?(?:\s+min_similarity (\d\.\d+))?", task_lower)
    if query_match:
        ...
        memories = manager.semantic_search(query_text, limit=limit, min_similarity=min_similarity)
    ...
    related_match = re.match(r"related to '(.+?)'(?:\s+type '(.+?)')?(?:\s+min_strength (\d\.\d+))?(?:\s+limit (\d+))?", task_lower)
    if related_match:
        ...
        related = manager.get_related_memories(key, relationship_type=rel_type, min_strength=min_strength, limit=limit)
        return {"status": "success", "related_memories": [mem.__dict__ for mem in related]}
    ...
    if history_match:
        ...
        return {"status": "success", "key": key, "history": [v.__dict__ for v in history]}
    ...
    if cleanup_match:
        ...
        manager.executor.submit(manager._cleanup_old_memories, days_override) # Run in background
        return {"status": "success", "message": f"Cleanup task submitted." + (f" (for memories older than {days_override} days)" if days_override else "")}
    ...
    if task_lower == "clear all memories --confirm":
        ...
        return {"status": "info", "message": "Confirmation needed. Use 'clear all memories --confirm'."}
...
if __name__ == '__main__':
    ...
    # Test write
    print(memory_manager_tool("write key 'goal1' value 'Achieve world peace' type 'ambition' importance 9.5 meta '{\"source\":\"test\"}'"))
    print(memory_manager_tool("write key 'task1' value 'Develop a universal translator' type 'project' importance 8"))
    print(memory_manager_tool("write key 'fact1' value 'The sky is blue on a clear day.' type 'observation' importance 3"))
    ...
    print(memory_manager_tool("write key 'goal1' value 'Achieve lasting world peace and prosperity' importance 9.8"))
    time.sleep(0.1)
    ...
    manager.add_memory("task1.1", "Research linguistic patterns", related_memories=[("task1", "subtask_of", 0.9)])
    time.sleep(0.1)
    ...
    print(memory_manager_tool("cleanup 0 days")) # Cleanup things older than 0 days (i.e. almost everything not critical)
    logger.info("Waiting for cleanup and decay tasks to potentially run...")
    time.sleep(test_config.refresh_interval_seconds + 5) # Wait for one maintenance cycle + buffer
