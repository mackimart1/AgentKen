...
def code_executor(code: str, language: str = 'python', timeout: int = 30) -> str:
    ...
    try:
        ...
        logger.info(f"Executing {validated_language} code with {validated_timeout}s timeout")
        tool_result_json = secure_code_executor(
            code=code, 
...
def _process_tool_result(result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if 'stdout' in result:
        output['stdout'] = str(result['stdout']) if result['stdout'] is not None else None
    
    # Process stderr (ensure it's a string)
    if 'stderr' in result:
        output['stderr'] = str(result['stderr']) if result['stderr'] is not None else None
