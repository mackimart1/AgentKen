...
class EnhancedReinforcementLearningManager:
    ...
    def create_enhanced_state(self, 
                           ...
                           system_load: float = 0.0) -> np.ndarray:
        ...
        state_features = [
            ...
            float(any(isinstance(msg, AIMessage) and msg.tool_calls for msg in current_messages)),  # Tool usage history
    ...
    def get_contextual_action(self, state_vector: np.ndarray, context: str = "") -> Tuple[int, str]:
        """Get action with contextual reasoning"""
        base_action = self.rl_manager.predict_action(state_vector)
        
        # Add contextual logic
        reasoning = ""
        if "error" in context.lower() and base_action == 0:  # If errors detected, consider retry
...
class MetaAnalyzer:
        ...
        try:
            ...
            for line in lines:
                if 'task completed' in line.lower():
                    ...
                    if agent_name:
                        metrics.agent_utilization[agent_name] = metrics.agent_utilization.get(agent_name, 0) + 1
    ...
    def generate_recommendations(self, metrics: AnalysisMetrics) -> List[str]:
        ...
        if metrics.agent_utilization:
            underused_agents = [agent for agent, count in metrics.agent_utilization.items() if count < 2]
            if underused_agents:
                recommendations.append(
        ...
        if metrics.error_patterns:
            frequent_errors = [(error, count) for error, count in metrics.error_patterns.items() if count > 5]
            if frequent_errors:
                recommendations.append(
    ...
    def _identify_unused_tools(self, tool_usage: Dict[str, int]) -> List[str]:
        ...
        try:
            ...
            all_tool_names = [getattr(tool, '__name__', str(tool)) for tool in all_tools]
...
def reasoning(state: MessagesState) -> dict:
    ...
    try:
        relevant_memories = memory_manager.retrieve_memories(query_text=query_context, k=10)
    ...
    try:
        ...
        rl_action, rl_reasoning = rl_manager.get_contextual_action(enhanced_state, query_context)
        ...
        if rl_action == 1:  # Retry with different approach
            ...
            current_messages = current_messages[-3:] if len(current_messages) > 3 else current_messages
        elif rl_action == 2:  # Abort
            print("🛑 RL suggests abort - task complexity too high")
            return {"messages": [AIMessage(content="Analysis aborted due to complexity assessment. Consider breaking down the task.")]}
    ...
    if relevant_memories:
        try:
            shortcut_response = check_memory_shortcuts(relevant_memories, task_description)
            if shortcut_response:
                print("⚡ Memory shortcut found - using optimized response")
                rl_manager.rl_manager.update_reward(task_completed=True, error_count=0, success_count=1)
    ...
    messages_for_llm = prepare_messages_for_llm(current_messages, relevant_memories, analyzer.metrics)
    ...
    if response:
        ...
        rl_manager.rl_manager.update_reward(task_completed=False, error_count=1, success_count=0)
        print("❌ Analysis failed")
    
    return {"messages": [response]} if response else {"messages": [AIMessage(content="Analysis failed to complete.")]}
...
def prepare_messages_for_llm(current_messages: List, memories: List[str], metrics: AnalysisMetrics) -> List:
    ...
    if metrics.total_tasks > 0:
        ...
        if metrics.agent_utilization:
            context_parts.append(f"• Active Agents: {list(metrics.agent_utilization.keys())}")
    ...
    if context_parts:
        ...
        if messages_for_llm and isinstance(messages_for_llm[0], SystemMessage):
            ...
            messages_for_llm[0] = SystemMessage(content=context_header + original_content)
        else:
            messages_for_llm.insert(0, SystemMessage(content=context_header + ENHANCED_SYSTEM_PROMPT))
    
    return messages_for_llm

def execute_llm_call_with_retry(messages: List, max_retries: int = 3) -> Optional[AIMessage]:
    ...
    while retry_count < max_retries:
        try:
            ...
            tooled_model = config.default_langchain_model.bind_tools(tools)
...
def generate_analysis_summary(final_state: Dict, execution_time: float) -> Dict[str, Any]:
...
if __name__ == '__main__':
    ...
    for i, test_task in enumerate(test_cases, 1):
        ...
        print(f"Execution Time: {result.get('metadata', {}).get('execution_time', 'N/A')}")
