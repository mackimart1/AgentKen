"""
Admin Test Agent: Agent created by admin
...
# Setup logger for admin_test_agent
...
system_prompt = """You are admin_test_agent, a ReAct agent in the AgentK system.
...
def admin_test_agent(task: str) -> Dict[str, Any]:
    """
    Admin Test Agent agent implementation.
    ...
    """
    try:
        logger.info(f"Starting admin_test_agent task: {task}")
        ...
        logger.info(f"admin_test_agent completed with status: {status}")

        return {"status": status, "result": result_name, "message": message}

    except Exception as e:
        logger.error(f"Error in admin_test_agent: {e}")
        return {
            "status": "failure",
            "result": None,
...
if __name__ == "__main__":
    # Test the agent
    test_task = "Test task for Admin Test Agent"
    result = admin_test_agent(test_task)
