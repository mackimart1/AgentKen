...
def check_tool_outcome(state: HermesState) -> Literal["reasoning"]:
    """
    Checks the outcome of the most recent tool call. Increments plan step count
    on successful agent assignment.

    Args:
        state (HermesState): The current graph state.

    Returns:
        dict | Literal["reasoning"]: Update for state (step count) or next node name.
    """
    ...
    if isinstance(last_message, ToolMessage) and isinstance(last_message.content, dict) and "status" in last_message.content:
        ...
        # Look backwards for the AIMessage with the tool call that led to this ToolMessage
        ...
        if tool_status == 'failure':
            ...
            try:
                ...
                logging.error(f"Failed to log assignment data to {log_file}: {log_e}")
            # --- End Logging ---

            # Add successful result to memory
            success_summary = f"Successfully executed {assigned_agent_name} on task '{original_task_info}'. Result/Message: {tool_result} / {tool_message}"
            memory_manager_instance.add_memory(
                key=f"solution_{datetime.datetime.now().timestamp()}",  # Generate unique key
