...
class RLConfig:
    ...
    target_update_interval: int = 10000 # For DQN, target network update frequency (timesteps)
...
class TaskState:
    ...
    agent_confidence: float = 0.5 # Agent's own confidence before RL decision, 0.0 to 1.0
    ...
    def to_vector(self) -> np.ndarray:
        ...
        return np.array([
            ...
            min(self.execution_time_steps / 100.0, 1.0), # Normalized execution time steps
...
class AdvancedTaskEnvironment(gym.Env):
    ...
    def __init__(self, config: Optional[RLConfig] = None, initial_task_state: Optional[TaskState] = None):
        ...
        self.action_space = spaces.Box(
            low=np.array([0, 0.0, 0.0]), # action_type is continuous, will be discretized
            high=np.array([4.99, 1.0, 1.0]), # action_type up to 4.99 to map to 0-4
            dtype=np.float32
        )
        
        self.observation_space = spaces.Box(
            low=0.0, high=1.0, # Assuming all features in TaskState.to_vector() are normalized
            shape=TaskState.get_observation_space_shape(),
            dtype=np.float32
        )
        
        self.current_task: TaskState = initial_task_state if initial_task_state else self._generate_random_task()
        self.episode_step = 0
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, bool, dict]:
        ...
        # Calculate reward
        reward = self._calculate_reward(action_type, param1, param2, prev_completion_status, prev_error_count)
        ...
        info = self._get_info()
        info.update({
            'action_type': action_type, 'param1': param1, 'param2': param2
        })
    ...
    def _update_task_state(self, action_type: int, param1: float, param2: float):
        ...
        if action_type == 0:  # Continue
            ...
            progress_increment = param1 * 0.1 * (self.current_task.agent_confidence + self.current_task.context_relevance) / 2
            self.current_task.completion_status = min(1.0, self.current_task.completion_status + progress_increment)
            if np.random.rand() < 0.1 * param1: # Chance of new error
                self.current_task.error_count +=1
            self.current_task.resource_usage = min(1.0, self.current_task.resource_usage + 0.01 * param1)

        elif action_type == 1:  # Retry
            self.current_task.retry_count += 1
            self.current_task.resource_usage = min(1.0, self.current_task.resource_usage + 0.02 * param1)
            ...
            if np.random.rand() < 0.2 * param1:
                 self.current_task.completion_status = min(1.0, self.current_task.completion_status + 0.05 * param1)
            ...
            # but costs resources.
            self.current_task.resource_usage = min(1.0, self.current_task.resource_usage + 0.05 * param1)
            ...
            if np.random.rand() < 0.3 * param1: # Help is effective
                 self.current_task.completion_status = min(1.0, self.current_task.completion_status + 0.1 * param1)
            ...
            self.current_task.agent_confidence = min(1.0, self.current_task.agent_confidence + 0.1 * param1)
            self.current_task.resource_usage = min(1.0, self.current_task.resource_usage + 0.01 * param1) # Small cost to optimize


    def _calculate_reward(self, action_type: int, param1: float, param2: float,
                          prev_completion_status: float, prev_error_count: int) -> float:
        reward = 0.0
        ...
        resource_efficiency = 1.0 - abs(self.current_task.resource_usage - optimal_resource_usage)
        ...
        if action_type == 1:  # Retry
            ...
            optimize_benefit = self.reward_weights['optimize_benefit'] * self.current_task.complexity_score
        ...
        if abs(self.current_task.completion_status - self._last_completion_status) < 0.01:
    ...
    def render(self, mode='human'):
        if mode == 'human':
            ...
            print(f"  Completion: {self.current_task.completion_status:.2f}, "
                  f"Errors: {self.current_task.error_count}, "
                  f"Retries: {self.current_task.retry_count}")
            print(f"  Resources: {self.current_task.resource_usage:.2f}, "
            ...
            if self.model.ep_info_buffer:
                ...
                if mean_reward > self.best_mean_reward:
                    ...
                    if self.save_path:
                        ...
                        if self.verbose > 0:
                            logger.info(f"New best model saved to {self.save_path} (mean reward: {self.best_mean_reward:.2f})")
        return True
...
class EnhancedReinforcementLearningManager:
    ...
    def _initialize_environment(self):
        try:
            if self.n_envs == 1:
                 ...
                 self.env = make_vec_env(self._env_fn, n_envs=self.n_envs, seed=self.config.seed,
    ...
    def train(self, total_timesteps: int = 100000, eval_freq: int = 5000, 
              checkpoint_freq: int = 10000, log_dir: str = "rl_training_logs") -> Dict[str, Any]:
        Path(log_dir).mkdir(parents=True, exist_ok=True)
        try:
            logger.info(f"Starting training for {total_timesteps} timesteps using {self.config.algorithm}.")
            start_time = time.time()
            
            # Callbacks
            eval_env = Monitor(AdvancedTaskEnvironment(config=self.config)) # Separate eval env
            eval_callback = EvalCallback(eval_env, best_model_save_path=str(Path(log_dir)/'best_model'),
                                         log_path=str(Path(log_dir)/'eval_logs'), eval_freq=max(eval_freq // self.n_envs, 1),
                                         deterministic=True, render=False, n_eval_episodes=5)
            
            progress_callback = TrainingProgressCallback(check_freq=max(checkpoint_freq // self.n_envs, 1), log_dir=log_dir)
            ...
            self.model.learn(
                ...
                progress_bar=True,
                log_interval=self.config.log_interval if self.config.algorithm in ["PPO", "A2C"] else 100, # DQN logs differently
    ...
    def predict_action(self, task_state: TaskState, deterministic: bool = True) -> Tuple[np.ndarray, Dict[str, Any]]:
        if self.model is None:
            logger.error("Model not initialized. Cannot predict.")
            # Return a default safe action (e.g., continue with medium params)
            return np.array([0, 0.5, 0.5], dtype=np.float32), {"error": "Model not initialized"}

        observation = task_state.to_vector().reshape(1, -1) # SB3 expects batch dimension
        ...
        action_type_raw, param1, param2 = action_values[0], action_values[1], action_values[2]
        ...
        try:
            ...
            if isinstance(self.env, DummyVecEnv):
                if 0 <= env_idx < self.n_envs:
                    self.env.envs[env_idx].set_next_task(task_state)
                else:
                    logger.warning(f"Invalid env_idx {env_idx} for {self.n_envs} environments.")
                ...
                logger.warning(f"set_task_for_env not implemented for VecEnv type: {type(self.env)}")
    ...
    def save_model(self, path: str):
        ...
        try:
            ...
            metadata = {
                ...
                'training_history_summary': {k: v[-1] if v else None for k, v in self.training_history.items()},
            ...
            logger.info(f"Model and metadata saved successfully to {path} and {metadata_path}")
        except Exception as e:
    ...
    def load_model(self, path: str, config_override: Optional[RLConfig] = None):
        try:
            ...
            if config_override:
                ...
                self.config = RLConfig(**metadata.get('config', {})) # Load config from metadata
                self.n_envs = metadata.get('n_envs', self.n_envs) # Load n_envs if available
            ...
            if self.config.algorithm == "PPO":
                self.model = PPO.load(model_path, env=self.env, device=self.config.device)
            elif self.config.algorithm == "A2C":
                self.model = A2C.load(model_path, env=self.env, device=self.config.device)
            elif self.config.algorithm == "DQN":
                self.model = DQN.load(model_path, env=self.env, device=self.config.device)
            else:
                 raise ValueError(f"Unsupported algorithm for loading: {self.config.algorithm}")
...
def plot_training_progress(log_dir: str = "rl_training_logs"):
    ...
    monitor_files = list(Path(log_dir).glob("**/monitor.csv")) # Search in eval_logs or others
    ...
    if not monitor_files:
        logger.warning(f"No monitor.csv files found in {log_dir} or its subdirectories.")
        return
    ...
    x, y = ts2xy(load_results(monitor_file_path), 'timesteps') # ep_rew_mean comes from Monitor logs
...
if __name__ == '__main__':
    ...
    try:
        ...
        training_results = rl_manager.train(total_timesteps=2000, eval_freq=500, checkpoint_freq=1000, log_dir=LOG_DIR)
        ...
        loaded_rl_manager = EnhancedReinforcementLearningManager(config=test_config, n_envs=1) # Config will be overridden by metadata if present
        loaded_rl_manager.load_model(str(save_path.with_suffix(".zip"))) # Pass .zip path

        loaded_action, loaded_metadata = loaded_rl_manager.predict_action(sample_task_state)
        logger.info(f"Loaded model predicted action: {loaded_action}, Metadata: {loaded_metadata}")
        
        assert np.allclose(action, loaded_action, atol=1e-5), "Action from loaded model differs!"
        ...
        try:
            ...
            logger.warning(f"Plotting test failed (matplotlib might be missing or no data): {e}")
