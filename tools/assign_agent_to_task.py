...
def assign_agent_to_task(agent_name: str, task: str):
    ...
    logger.info(f"Assigning task to agent '{agent_name}'. Timeout: {AGENT_EXECUTION_TIMEOUT}s")
    ...
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=1, thread_name_prefix=f"agent_exec_{agent_name}") as executor:
            ...
            try:
                ...
                logger.warning(f"Attempted to cancel timed-out agent '{agent_name}'. Cancel successful: {cancelled}")
                return {'status': 'failure', 'result': None, 'message': error_message}
        ...
        if not isinstance(structured_result, dict) or 'status' not in structured_result:
            logger.warning(f"Agent '{agent_name}' returned an unexpected format: {structured_result}")
            return {'status': 'unknown', 'result': structured_result, 'message': f"Agent response format might be outdated or missing 'status': {str(structured_result)[:500]}"}

        # --- File Creation Verification (for agent_smith and tool_maker) ---
        if agent_name in ['agent_smith', 'tool_maker'] and structured_result.get('status') == 'success':
            ...
            if created_name:
                ...
                if expected_path and not os.path.exists(expected_path):
                    ...
                    structured_result['result'] = None
                     ...
                     logger.info(f"Verified file exists for '{created_name}': {expected_path}")
                else:
                     # This case shouldn't happen if agent_name is one of the two, but handle defensively
                     logger.warning(f"Could not determine expected path for agent '{agent_name}' result '{created_name}'. Skipping file verification.")

            else:
                # Agent reported success but didn't return the name of the created entity
                logger.warning(f"Agent '{agent_name}' reported success but did not return the name of the created entity in the 'result' field. Cannot verify file creation.")
                ...
                structured_result['message'] = structured_result.get('message', '') + " (Warning: Could not verify file creation as agent result was missing)."


        # --- Final Return ---
        print(f"Agent '{agent_name}' responded with status: {structured_result.get('status')}")
        if structured_result.get('message'):
            print(f"Message: {str(structured_result['message'])[:500]}{'...' if len(str(structured_result['message'])) > 500 else ''}")
