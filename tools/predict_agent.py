...
try:
    if os.path.exists(MODEL_PATH):
        ...
        logger.warning(f"Agent selector model not found at {MODEL_PATH}. Prediction tool will be disabled.")

    if os.path.exists(ENCODER_PATH):
        label_encoder = joblib.load(ENCODER_PATH)
        logger.info(f"Loaded label encoder from {ENCODER_PATH}")
    else:
        logger.warning(f"Label encoder not found at {ENCODER_PATH}. Prediction tool will be disabled.")
        agent_selector_model = None # Disable model if encoder is missing

    if agent_selector_model and label_encoder: # Only load embedder if model/encoder loaded
...
class PredictAgentInput(BaseModel):
    task_description: str = Field(description="The description of the task to be assigned.")
...
def predict_agent(task_description: str) -> Dict[str, float]:
    ...
    try:
        ...
        task_embedding = embedding_model.encode([task_description]) # Encode expects a list
        ...
        logger.info(f"Predicted agent probabilities for task '{task_description[:50]}...': {agent_probabilities}")
...
if __name__ == "__main__":
    ...
    if agent_selector_model and label_encoder and embedding_model:
        ...
        if probs_2:
            sorted_probs_2 = dict(sorted(probs_2.items(), key=lambda item: item[1], reverse=True))
            print("Probabilities:")
