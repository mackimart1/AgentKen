...
def _create_session(session_id: str, working_directory: Optional[str], environment_vars: Optional[Dict[str, str]]) -> Dict[str, Any]:
...
def _execute_command(session_id: str, command: Optional[str], timeout: int, multi_command: bool) -> Dict[str, Any]:
...
def _execute_single_command(session: TerminalSession, command: str, timeout: int) -> Dict[str, Any]:
...
def _run_subprocess_command(command: str, session: TerminalSession, env: Dict[str, str], timeout: int, start_time: float) -> Dict[str, Any]:
...
def _execute_multiple_commands(session: TerminalSession, commands: List[str], timeout: int) -> Dict[str, Any]:
    ...
    overall_status = "success" if failed_commands == 0 else "partial" if successful_commands > 0 else "failure"
