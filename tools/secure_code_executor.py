...
def secure_code_executor(code: str, language: str = 'python') -> str:
    ...
    if language.lower() != 'python':
        output["message"] = f"Unsupported language: {language}. Only Python is currently supported."
    ...
    try:
        ...
        if process.returncode == 0:
            ...
            output["message"] = f"Code execution failed with return code {process.returncode}."
            # Include stderr in the error field for easier debugging if execution failed
            output["error"] = process.stderr if process.stderr else "Execution failed with non-zero exit code."
        ...
        output["status"] = "failure"
        output["error"] = str(e)
        output["message"] = f"An error occurred during code execution setup or processing: {e}"
    finally:
