...
def evaluate_binary_image_classifier(
    ...
    try:
        ...
        if not isinstance(target_size, list) or len(target_size) != 2:
             return {"error": "target_size must be a list of two integers [height, width]"}
        ...
        if len(true_classes) != len(predicted_classes):
             return {"error": f"Mismatch between number of true labels ({len(true_classes)}) and predictions ({len(predicted_classes)}). Check generator steps or data."}
        ...
        if cm.shape != (2, 2):
             ...
             if len(np.unique(true_classes)) == 1 or len(np.unique(predicted_classes)) == 1:
                 cm_full = np.zeros((2, 2), dtype=int)
                 present_class = np.unique(true_classes)[0] # Assume true classes dictate the present class
                 if present_class == 0: # Only class 0 present
                     cm_full[0, 0] = cm[0,0] if cm.shape == (1,1) else cm[0] if cm.ndim == 1 else 0 # TN
                 else: # Only class 1 present
                     cm_full[1, 1] = cm[0,0] if cm.shape == (1,1) else cm[0] if cm.ndim == 1 else 0 # TP
                 cm = cm_full
             else:
                 # Fallback for unexpected shapes, though less likely in binary case
                 logger.error(f"Unexpected confusion matrix shape: {cm.shape}. Returning as is.")
