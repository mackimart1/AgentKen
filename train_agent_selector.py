...
def load_data(filepath: str) -> pd.DataFrame:
    ...
    try:
        ...
        if 'task' not in df.columns or 'agent' not in df.columns:
             ...
             return None
        df.dropna(subset=['task', 'agent'], inplace=True) # Drop rows with missing crucial data
...
def preprocess_data(df: pd.DataFrame, embedding_model: SentenceTransformer, encoder_save_path: str):
    ...
    task_embeddings = embedding_model.encode(df['task'].tolist(), show_progress_bar=True)
...
if __name__ == "__main__":
    ...
    if len(data) < BATCH_SIZE * 2: # Need enough data for at least one train/test batch
         logger.error(f"Not enough data to train (found {len(data)} records). Need at least {BATCH_SIZE * 2}. Exiting.")
         exit()
    ...
    if X is None or y is None or n_classes is None:
        ...
        exit()
    if n_classes <= 1:
        logger.error(f"Only found {n_classes} agent class(es) in the data. Need at least 2 for classification. Exiting.")
        exit()
    ...
    X_train, X_test, y_train, y_test = train_test_split(
        X, y, test_size=TEST_SPLIT_SIZE, random_state=RANDOM_STATE, stratify=y # Stratify for balanced classes
    )
    ...
    history = model.fit(
        ...
        callbacks=[tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=3, restore_best_weights=True)] # Stop early if no improvement
    )
