...
class AlertManager:
    ...
    def add_alert_rule(self, metric_name: str, threshold: float, 
                      ...
                      description: str = "", window_seconds: int = 60):
        """Add an alert rule"""
    ...
    def _create_alert(self, rule_id: str, rule: Dict[str, Any], current_value: float):
        ...
        alert = Alert(
            ...
            description=rule.get('description', f"Metric {rule['metric_name']} {rule['condition']} {rule['threshold']}"),
            metric_name=rule['metric_name'],
            threshold=rule['threshold'],
...
class BottleneckDetector:
    ...
    def analyze_bottlenecks(self, window_seconds: int = 300) -> List[BottleneckAnalysis]:
    ...
    def _detect_latency_bottlenecks(self, window_seconds: int) -> List[BottleneckAnalysis]:
        """Detect latency-based bottlenecks"""
        bottlenecks = []
        
        # Look for components with high latency
        latency_metrics = [name for name in self.metrics_collector.metrics.keys() 
    ...
    def _detect_throughput_bottlenecks(self, window_seconds: int) -> List[BottleneckAnalysis]:
        """Detect throughput-based bottlenecks"""
        bottlenecks = []
        
        # Look for components with declining throughput
        throughput_metrics = [name for name in self.metrics_collector.metrics.keys()
    ...
    def _detect_resource_bottlenecks(self, window_seconds: int) -> List[BottleneckAnalysis]:
        """Detect resource utilization bottlenecks"""
        bottlenecks = []
        
        # Look for high resource utilization
        resource_metrics = [name for name in self.metrics_collector.metrics.keys()
    ...
    def _detect_queue_bottlenecks(self, window_seconds: int) -> List[BottleneckAnalysis]:
        """Detect queue-based bottlenecks"""
        bottlenecks = []
        
        # Look for growing queues
        queue_metrics = [name for name in self.metrics_collector.metrics.keys()
...
class OptimizationEngine:
    ...
    def generate_recommendations(self, window_seconds: int = 300) -> List[OptimizationRecommendation]:
    ...
    def _create_recommendation_for_bottleneck(self, bottleneck: BottleneckAnalysis) -> Optional[OptimizationRecommendation]:
        ...
        strategy = strategy_map.get(bottleneck.bottleneck_type, OptimizationStrategy.SCALE_UP)
        ...
        return OptimizationRecommendation(
            ...
            expected_improvement=min(bottleneck.severity_score * 10, 50),  # Up to 50% improvement
    ...
    def _generate_proactive_recommendations(self, window_seconds: int) -> List[OptimizationRecommendation]:
    ...
    def _is_trending_upward(self, metric_name: str, window_seconds: int) -> bool:
        ...
        recent_metrics = self.metrics_collector.get_metrics(metric_name, recent_start, end_time)
        older_metrics = self.metrics_collector.get_metrics(metric_name, older_start, recent_start)
    ...
    def _create_proactive_recommendation(self, metric_name: str) -> Optional[OptimizationRecommendation]:
...
class WorkflowMonitor:
    ...
    def __init__(self):
        ...
        self.optimization_engine = OptimizationEngine(self.metrics_collector, self.bottleneck_detector)
    ...
    def add_baseline(self, metric_name: str, baseline_value: float, 
                    acceptable_deviation: float = 0.2, measurement_window: int = 100):
    ...
    def get_optimization_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'high_priority_recommendations': [r for r in recommendations if r.priority_score > 7.0],
    ...
    def simulate_load(self, duration_seconds: int = 60):
        ...
        def load_simulator():
            ...
            while time.time() - start_time < duration_seconds:
                ...
                load_factor = 1 + (time.time() - start_time) / duration_seconds  # Increase over time
                ...
                # Simulate throughput
                throughput = max(1, 50 - (time.time() - start_time) / 2)  # Decrease over time
                self.metrics_collector.record_metric(Metric(
                    name="app_throughput",
                ...
                # Simulate error rate
                error_rate = min(20, (time.time() - start_time) / 3)  # Increase over time
                self.metrics_collector.record_metric(Metric(
                    name="app_error_rate",
