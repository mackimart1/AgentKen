...
class LegacyToolWrapper(BaseTool):
    ...
    def _extract_input_schema(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logging.warning(f"Could not extract schema for {self.legacy_info.name}: {e}")
            return {"input": "string"}
...
class LegacyAgentWrapper(BaseAgent):
    ...
    def __init__(self, legacy_agent_info: LegacyAgentInfo, legacy_function: Callable, message_bus):
        ...
        for cap_name in legacy_agent_info.capabilities:
            capability = AgentCapability(
                ...
                output_schema={"status": "string", "result": "any", "message": "string"}
            )
...
class LegacyIntegrationManager:
    ...
    def discover_legacy_components(self):
        ...
        self.logger.info(f"Discovered {len(self.legacy_agents)} agents and {len(self.legacy_tools)} tools")
    ...
    def _discover_legacy_agents(self):
            ...
            try:
                with open(manifest_path, 'r') as f:
                    ...
                    if isinstance(manifest, list):
                        agents_from_manifest = {agent['name']: agent for agent in manifest}
                    elif isinstance(manifest, dict):
                        agents_from_manifest = manifest.get('agents', {})
                    else:
                        agents_from_manifest = {}
                self.logger.info(f"Loaded agents manifest with {len(agents_from_manifest)} entries")
            except Exception as e:
        ...
        for agent_file in self.agents_path.glob("*.py"):
            ...
            if agent_name in agents_from_manifest:
                ...
                description = agent_info.get('description', f'Legacy agent: {agent_name}')
            ...
            try:
                with open(manifest_path, 'r') as f:
                    ...
                    if isinstance(manifest, list):
                        ...
                        tools_from_manifest = {}
                self.logger.info(f"Loaded tools manifest with {len(tools_from_manifest)} entries")
            except Exception as e:
    ...
    def load_legacy_function(self, module_path: str, function_name: str) -> Optional[Callable]:
        ...
        for agent_name, agent_info in self.legacy_agents.items():
            try:
                ...
                legacy_function = self.load_legacy_function(
                    ...
                    agent_info.function_name
                )
                
                if legacy_function:
                    # Create wrapper
                    wrapper = LegacyAgentWrapper(agent_info, legacy_function, message_bus)
                    self.wrapped_agents[agent_name] = wrapper
                    self.logger.info(f"Wrapped agent: {agent_name}")
                else:
                    self.logger.warning(f"Could not load function for agent: {agent_name}")
        ...
        for tool_name, tool_info in self.legacy_tools.items():
            try:
                ...
                if legacy_function:
                    ...
                    self.logger.warning(f"Could not load function for tool: {tool_name}")
    ...
    def register_with_enhanced_system(self, tool_registry: ToolRegistry, agent_list: List):
...
def integrate_legacy_components(tool_registry: ToolRegistry, agent_list: List, message_bus) -> Dict[str, Any]:
    """
