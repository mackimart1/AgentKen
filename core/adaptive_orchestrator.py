...
class PlanOptimizer:
    ...
    def optimize_plan(self, plan: ExecutionPlan, 
                     ...
                     resource_constraints: Dict[str, ResourceConstraint]) -> ExecutionPlan:
        ...
        for strategy in self.optimization_strategies:
            try:
                optimized_plan = strategy(optimized_plan, agent_capacities, resource_constraints)
    ...
    def _optimize_resource_allocation(self, plan: ExecutionPlan,
                                    agent_capacities: Dict[str, AgentCapacity],
                                    resource_constraints: Dict[str, ResourceConstraint]) -> ExecutionPlan:
        """Optimize resource allocation across tasks"""
        
        # Sort tasks by priority and resource requirements
        sorted_tasks = sorted(plan.tasks, 
                            key=lambda t: (t.priority.value, len(t.required_resources)),
                            reverse=True)
        
        # Allocate resources optimally
        for task in sorted_tasks:
            best_allocation = self._find_best_resource_allocation(task, resource_constraints)
            if best_allocation:
                task.required_resources = best_allocation
        
        return plan
    
    def _optimize_agent_assignment(self, plan: ExecutionPlan,
                                 agent_capacities: Dict[str, AgentCapacity],
                                 resource_constraints: Dict[str, ResourceConstraint]) -> ExecutionPlan:
    ...
    def _optimize_task_ordering(self, plan: ExecutionPlan,
                              agent_capacities: Dict[str, AgentCapacity],
                              resource_constraints: Dict[str, ResourceConstraint]) -> ExecutionPlan:
        ...
        for task in tasks:
            ...
            for dep in task.dependencies:
                ...
                if dep in task_ids:
                    ...
                    logging.warning(f"Task {task.id} depends on non-existent task {dep}")
        
        return dict(graph)
    
    def _find_parallel_groups(self, dependency_graph: Dict[str, Set[str]]) -> List[Set[str]]:
    ...
    def _find_best_resource_allocation(self, task: Task, 
                                     resource_constraints: Dict[str, ResourceConstraint]) -> Optional[List[str]]:
    ...
    def _select_optimal_agent(self, task: Task, 
                            agent_capacities: Dict[str, AgentCapacity]) -> Optional[str]:
        ...
        while ready_queue:
            ...
            for dependent_id in adj_list[task_id]:
                ...
                if in_degree[dependent_id] == 0:
                    ...
                    heapq.heappush(ready_queue, (-dependent_task.priority.value, dependent_id))
...
class AdaptivePlanner:
    ...
    def adapt_plan(self, plan: ExecutionPlan, 
                  execution_context: Dict[str, Any]) -> Optional[ExecutionPlan]:
        ...
        logging.info(f"Plan {plan.id} adapted (adaptation #{adapted_plan.adaptation_count})")
        
        return adapted_plan
    
    def _assess_adaptation_need(self, plan: ExecutionPlan, 
    ...
    def _create_adapted_plan(self, original_plan: ExecutionPlan, 
                           execution_context: Dict[str, Any]) -> ExecutionPlan:
    ...
    def _handle_resource_shortage(self, plan: ExecutionPlan, context: Dict[str, Any]):
        ...
        for resource_name in resource_shortages:
            ...
            affected_tasks = [t for t in plan.tasks if resource_name in t.required_resources]
            
            # Reschedule to spread resource usage over time
            for i, task in enumerate(affected_tasks):
                task.estimated_duration += i * 5  # Add delay
    
    def _handle_performance_degradation(self, plan: ExecutionPlan, context: Dict[str, Any]):
...
class AdvancedOrchestrator:
    ...
    def __init__(self, message_bus, tool_registry):
        ...
        self.execution_monitor = threading.Thread(target=self._monitor_execution, daemon=True)
    ...
    def create_plan(self, name: str, description: str, tasks: List[Dict[str, Any]], 
                   deadline: Optional[float] = None, priority: TaskPriority = TaskPriority.NORMAL) -> str:
        ...
        for i, task_data in enumerate(tasks):
            task = Task(
                ...
                dependency_type=DependencyType(task_data.get('dependency_type', 'sequential')),
    ...
    def _execute_plan_tasks(self, plan: ExecutionPlan):
        ...
        try:
            ...
            plan.efficiency_score = plan.original_estimated_duration / plan.actual_duration
    ...
    def _build_execution_schedule(self, plan: ExecutionPlan) -> List[List[Task]]:
        ...
        while remaining_tasks:
            ...
            parallel_batch = [t for t in ready_tasks if t.dependency_type == DependencyType.PARALLEL]
            sequential_tasks = [t for t in ready_tasks if t.dependency_type != DependencyType.PARALLEL]
    ...
    def _execute_task(self, task: Task, plan: ExecutionPlan) -> Any:
        ...
        try:
            ...
            if task.capability in self.tool_registry.tools:
                result = self.tool_registry.execute_tool(task.capability, **task.parameters)
            else:
    ...
    def get_system_metrics(self) -> Dict[str, Any]:
        ...
        return {
            'active_plans': len([p for p in self.active_plans.values() 
                               if p.status == PlanStatus.IN_PROGRESS]),
            'total_plans': len(self.active_plans),
            'agent_utilization': {agent_id: capacity.current_tasks / capacity.max_concurrent
                                for agent_id, capacity in self.agent_capacities.items()},
            'resource_utilization': {name: constraint.current_usage / constraint.capacity
