...
class BaseAgent(ABC):
    ...
    def __init__(self, agent_id: str, capabilities: List[AgentCapability], message_bus: MessageBus):
        ...
        self.message_bus.subscribe(MessageType.CAPABILITY_QUERY, self._handle_capability_query)
        self.message_bus.subscribe(MessageType.HEARTBEAT, self._handle_heartbeat)
        
        self.status = AgentStatus.IDLE
        logging.info(f"Agent {self.agent_id} initialized with {len(capabilities)} capabilities")
    ...
    def _handle_task_request(self, message: Message):
        ...
        if task_type not in self.capabilities:
            self._send_error_response(message, f"Capability '{task_type}' not supported")
            return
        ...
        if current_tasks >= capability.max_concurrent:
            self._send_error_response(message, f"Max concurrent tasks ({capability.max_concurrent}) reached")
            return
    ...
    def _execute_task(self, task_id: str, task_type: str, payload: Dict[str, Any]) -> TaskResult:
...
class EnhancedOrchestrator:
    ...
    def __init__(self, message_bus: MessageBus):
        ...
        self.message_bus.subscribe(MessageType.TASK_RESPONSE, self._handle_task_response)
        self.message_bus.subscribe(MessageType.ERROR_REPORT, self._handle_error_report)
        self.message_bus.subscribe(MessageType.CAPABILITY_RESPONSE, self._handle_capability_response)
        self.message_bus.subscribe(MessageType.HEARTBEAT, self._handle_heartbeat)
    ...
    def discover_agents(self) -> Dict[str, List[AgentCapability]]:
        ...
        return {agent_id: info.get('capabilities', []) 
    ...
    def _select_best_agent(self, task_type: str) -> Optional[str]:
        ...
        for agent_id, info in self.agents.items():
            ...
            max_concurrent = next((cap.get('max_concurrent', 1) 
                                 for cap in capabilities if cap.get('name') == task_type), 1)
    ...
    def _handle_task_response(self, message: Message):
        ...
        if result_data:
            ...
            logging.info(f"Task {task_id} completed by {message.sender}: "
                        f"{'SUCCESS' if result.success else 'FAILED'}")
    
    def _handle_error_report(self, message: Message):
        """Handle error reports from agents"""
        logging.error(f"Error report from {message.sender}: {message.payload.get('error')}")
    ...
    def _handle_heartbeat(self, message: Message):
        ...
        if agent_id in self.agents:
            ...
            self.agents[agent_id]['active_tasks'] = message.payload.get('active_tasks', 0)
...
if __name__ == "__main__":
    ...
    task_id = orchestrator.submit_task(
        "web_research",
        {"query": "AI agent frameworks", "depth": 3}
    )
