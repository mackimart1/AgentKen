...
from core.workflow_monitoring import WorkflowMonitor, console_notification_handler, log_notification_handler
from core.tool_integration_system import create_tool_system
from core.agent_framework import create_agent_system
from core.adaptive_orchestrator import AdvancedOrchestrator, create_sample_plan, AgentCapacity, ResourceConstraint
...
def main():
    ...
    logging.info("Tool integration system initialized with %d tools.", len(tool_registry.tools))
    ...
    logging.info(f"  - Agents: {integration_report['wrapped_agents']}/{integration_report['discovered_agents']}")
    logging.info(f"  - Tools: {integration_report['wrapped_tools']}/{integration_report['discovered_tools']}")
    ...
    try:
        ...
        logging.info("\n=== OPTIMIZATION REPORT ===")
        logging.info(f"System Health Score: {optimization_report['system_health_score']:.1f}/100")
        logging.info(f"Active Alerts: {optimization_report['active_alerts_count']}")
        logging.info(f"Bottlenecks Detected: {optimization_report['bottlenecks_detected']}")
