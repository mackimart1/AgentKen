...
class TestRunShellCommand(unittest.TestCase):
    ...
    def test_run_command_success(self, mock_time, mock_subprocess_run):
        ...
        expected_keys = ["status", "stdout", "stderr", "returncode", "message", "execution_time"]
    ...
    def test_run_command_subprocess_error(self, mock_time, mock_subprocess_run):
        ...
        mock_subprocess_run.side_effect = subprocess.SubprocessError("Failed to start process")
    ...
    @patch('time.time')
    @patch('tools.run_shell_command.os.path.exists')
    def test_custom_working_directory(self, mock_exists, mock_time, mock_subprocess_run):
