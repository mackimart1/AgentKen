...
np_mock = MagicMock() # Mock numpy if needed for specific checks, though usually direct use is fine
...
metrics_mock.confusion_matrix = MagicMock(return_value=np.array([[50, 5], [10, 35]])) # TN=50, FP=5, FN=10, TP=35
...
class TestEvaluateBinaryImageClassifier(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_generator_instance.flow_from_directory.return_value = self.mock_test_generator
        # Ensure the ImageDataGenerator constructor returns the instance that has flow_from_directory
        tf_mock.keras.preprocessing.image.ImageDataGenerator.return_value = self.mock_generator_instance
    ...
    def test_successful_evaluation(self):
        ...
        result = evaluate_binary_image_classifier.evaluate_binary_image_classifier.invoke({
        ...
        tf_mock.keras.preprocessing.image.ImageDataGenerator.assert_called_once_with(rescale=1./255)
        ...
        self.mock_model.predict.assert_called_once_with(self.mock_test_generator, steps=expected_steps)
        ...
        np.testing.assert_array_equal(metrics_mock.accuracy_score.call_args[0][0], self.true_classes)
        np.testing.assert_array_equal(metrics_mock.accuracy_score.call_args[0][1], expected_predicted_classes)

        metrics_mock.precision_score.assert_called_once()
        np.testing.assert_array_equal(metrics_mock.precision_score.call_args[0][0], self.true_classes)
        np.testing.assert_array_equal(metrics_mock.precision_score.call_args[0][1], expected_predicted_classes)
        self.assertEqual(metrics_mock.precision_score.call_args[1]['zero_division'], 0) # Check kwarg

        metrics_mock.recall_score.assert_called_once()
        np.testing.assert_array_equal(metrics_mock.recall_score.call_args[0][0], self.true_classes)
        np.testing.assert_array_equal(metrics_mock.recall_score.call_args[0][1], expected_predicted_classes)
        self.assertEqual(metrics_mock.recall_score.call_args[1]['zero_division'], 0) # Check kwarg

        metrics_mock.f1_score.assert_called_once()
        np.testing.assert_array_equal(metrics_mock.f1_score.call_args[0][0], self.true_classes)
        np.testing.assert_array_equal(metrics_mock.f1_score.call_args[0][1], expected_predicted_classes)
        self.assertEqual(metrics_mock.f1_score.call_args[1]['zero_division'], 0) # Check kwarg

        metrics_mock.confusion_matrix.assert_called_once()
        np.testing.assert_array_equal(metrics_mock.confusion_matrix.call_args[0][0], self.true_classes)
        np.testing.assert_array_equal(metrics_mock.confusion_matrix.call_args[0][1], expected_predicted_classes)
    ...
    def test_prediction_error(self):
        ...
        self.assertIn("error", result)
        self.assertTrue("An unexpected error occurred: Prediction failed!" in result["error"])

    def test_metrics_calculation_error(self):
        """ Test error handling when a scikit-learn metric function raises an exception """
        metrics_mock.accuracy_score.side_effect = Exception("Metrics calculation failed!")
        result = evaluate_binary_image_classifier.evaluate_binary_image_classifier.invoke({
            "saved_model_path": "/fake/model.h5",
            "test_data_dir": "/fake/test_data",
            "target_size": [150, 150]
        ...
        self.assertIn("error", result)
        self.assertTrue("An unexpected error occurred: Metrics calculation failed!" in result["error"])
    ...
    def test_mismatched_labels_predictions(self):
        ...
        self.mock_model.predict.return_value = np.random.rand(99, 1) # 99 predictions instead of 100
        result = evaluate_binary_image_classifier.evaluate_binary_image_classifier.invoke({
            "saved_model_path": "/fake/model.h5",
            "test_data_dir": "/fake/test_data",
            "target_size": [150, 150]
