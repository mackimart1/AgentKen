...
if not os.path.exists(tool_module_path):
    ...
    print(f"Warning: Tool file not found at {tool_module_path}. Creating dummy file for import.", file=sys.stderr)
...
try:
    ...
    from tools.calculator import calculator as calculator_tool_func
     ...
     raise ImportError(f"Could not import calculator tool from '{tool_module_path}'. Check file structure, sys.path, and potential errors in the tool file itself. Error: {e}")
...
class TestCalculator(unittest.TestCase):
    ...
    def test_addition(self):
        ...
        self.assertEqual(calculator_tool_func.invoke({"expression": " 2+3 "}), 5.0) # Test with spaces
    ...
    def test_float_division(self):
        self.assertAlmostEqual(calculator_tool_func.invoke({"expression": "10 / 3"}), 3.3333333333333335)

    def test_order_of_operations(self):
        self.assertEqual(calculator_tool_func.invoke({"expression": "2 + 3 * 4"}), 14.0)
        self.assertEqual(calculator_tool_func.invoke({"expression": "(2 + 3) * 4"}), 20.0)

    def test_division_by_zero(self):
        self.assertEqual(calculator_tool_func.invoke({"expression": "1 / 0"}), "Error: Division by zero.")
        self.assertEqual(calculator_tool_func.invoke({"expression": "1 / (2 - 2)"}), "Error: Division by zero.")
    ...
    def test_invalid_expression_syntax(self):
        ...
        result_paren = calculator_tool_func.invoke({"expression": "(2 + 3"}) # Missing closing parenthesis
        self.assertTrue(result_paren.startswith("Invalid syntax:"))
        result_op = calculator_tool_func.invoke({"expression": "2+"}) # Incomplete expression
    ...
    def test_invalid_expression_name(self):
        ...
        result_os = calculator_tool_func.invoke({"expression": "os.system('echo hello')"})
        self.assertTrue("Use of disallowed name 'os'" in result_os or "Invalid expression: NameError" in result_os)

        result_print = calculator_tool_func.invoke({"expression": "print('hello')"})
        self.assertTrue("Use of disallowed name 'print'" in result_print or "Invalid expression: NameError" in result_print)

        # Test disallowed builtins (e.g. __import__) - should be caught by invalid char check first
        result_import = calculator_tool_func.invoke({"expression": "__import__('os').system('echo hello')"})
    ...
    def test_complex_expression(self):
        self.assertEqual(calculator_tool_func.invoke({"expression": "100 / (2 + 3) * 2 - 1"}), 39.0)
        self.assertEqual(calculator_tool_func.invoke({"expression": "100 / ((1+1) + 3) * 2 - 1"}), 39.0)
    ...
    def test_negative_numbers(self):
        ...
        self.assertEqual(calculator_tool_func.invoke({"expression": "-10 / -2"}), 5.0)
        self.assertEqual(calculator_tool_func.invoke({"expression": "10 + (-2)"}), 8.0) # Parentheses with negative


    def test_invalid_chars(self):
         self.assertEqual(calculator_tool_func.invoke({"expression": "1;print(1)"}), "Error: Invalid characters (;) in expression.")
         self.assertEqual(calculator_tool_func.invoke({"expression": "1+__import__('os')"}) , "Error: Invalid characters (__) in expression.")
    ...
    def test_large_numbers(self):
         ...
         self.assertEqual(calculator_tool_func.invoke({"expression": "100000000000000000000 * 2"}), 2e+21)
    ...
    def test_spaces(self):
        self.assertEqual(calculator_tool_func.invoke({"expression": "  2 +   3  "}), 5.0)
        self.assertEqual(calculator_tool_func.invoke({"expression": "5*6"}), 30.0) # No spaces
