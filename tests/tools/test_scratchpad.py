...
try:
    ...
    raise ImportError(f"Could not import scratchpad tool from {tools_dir}. Check PYTHONPATH and file location. sys.path: {sys.path}") from e
...
class TestScratchpadTool(unittest.TestCase):
    ...
    def setUp(self):
        ...
        try:
            ...
            if isinstance(result, dict) and result.get('status') != 'success':
                 print(f"Warning: setUp clear_all API call did not return success: {result}")
        except Exception as e:
            # If the invoke call itself fails, it's a critical setup error
            self.fail(f"Critical Error: setUp clear_all failed during tool invocation: {e}")
    ...
    def test_write_and_read(self):
        ...
        write_result = scratchpad_tool.invoke({'action': 'write', 'key': 'test_key_1', 'value': 'test_value_1'})
    ...
    def test_read_nonexistent(self):
        ...
        read_result = scratchpad_tool.invoke({'action': 'read', 'key': 'nonexistent_key'})
    ...
    def test_delete_key(self):
        ...
        # Write a key
        scratchpad_tool.invoke({'action': 'write', 'key': 'to_delete', 'value': 'delete_me'})
        ...
        delete_result_again = scratchpad_tool.invoke({'action': 'delete', 'key': 'to_delete'})
        self.assertEqual(delete_result_again, {'status': 'key_not_found', 'key': 'to_delete'})
    ...
    def test_invalid_action_or_args(self):
        ...
        self.assertIsInstance(result_invalid, dict, "Result should be a dict for invalid action")
        # Check for common error indicators in the API response
        self.assertTrue(
            result_invalid.get('status') == 'error' or 'invalid' in result_invalid.get('message', '').lower(),
        ...
        result_write_no_key = scratchpad_tool.invoke({'action': 'write', 'value': 'some_value'})
        self.assertIsInstance(result_write_no_key, dict, "Result should be a dict for missing key in write")
        self.assertTrue(
            result_write_no_key.get('status') == 'error' or 'missing' in result_write_no_key.get('message', '').lower() or 'required' in result_write_no_key.get('message', '').lower(),
        ...
        result_write_no_value = scratchpad_tool.invoke({'action': 'write', 'key': 'some_key'})
        self.assertIsInstance(result_write_no_value, dict, "Result should be a dict for missing value in write")
        self.assertTrue(
            result_write_no_value.get('status') == 'error' or 'missing' in result_write_no_value.get('message', '').lower() or 'required' in result_write_no_value.get('message', '').lower(),
        ...
        self.assertIsInstance(result_read_no_key, dict, "Result should be a dict for missing key in read")
        self.assertTrue(
            result_read_no_key.get('status') == 'error' or 'missing' in result_read_no_key.get('message', '').lower() or 'required' in result_read_no_key.get('message', '').lower(),
        ...
        self.assertIsInstance(result_delete_no_key, dict, "Result should be a dict for missing key in delete")
        self.assertTrue(
            result_delete_no_key.get('status') == 'error' or 'missing' in result_delete_no_key.get('message', '').lower() or 'required' in result_delete_no_key.get('message', '').lower(),
