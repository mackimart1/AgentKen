...
class TestFeatureEngineerForAgentSelection(unittest.TestCase):
    ...
    def test_basic_feature_structuring(self):
        ...
        agent_meta = [
            {"name": "AnalystAgent", "capabilities": ["data_analysis", "reporting"], "type": "specialist"},
            {"name": "WebSearchAgent", "capabilities": ["web_search", "summarization"], "type": "generalist"},
        ]
        ...
        self.assertEqual(result["task_features"]["description"], expected_output["task_features"]["description"])
        self.assertEqual(result["task_features"]["description_length"], expected_output["task_features"]["description_length"])
        self.assertEqual(result["agent_features"]["count"], expected_output["agent_features"]["count"])
        self.assertListEqual(result["agent_features"]["metadata_list"], expected_output["agent_features"]["metadata_list"])
        self.assertDictEqual(result["context_features"], expected_output["context_features"])
