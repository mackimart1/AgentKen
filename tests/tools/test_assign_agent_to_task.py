...
class TestAssignAgentToTask(unittest.TestCase):
    ...
    def test_assign_agent_failure_status(self, mock_load_module):
        ...
        result = assign_agent_to_task.invoke({"agent_name": "failing_agent", "task": "Task"})
    ...
    def test_assign_agent_execution_exception(self, mock_load_module):
        ...
        result = assign_agent_to_task.invoke({"agent_name": "error_agent", "task": "Task"})

        self.assertEqual(result["status"], "failure")
        self.assertIsNone(result["result"])
        self.assertIn("An error occurred while executing agent error_agent", result["message"])
        self.assertIn("Agent runtime error!", result["message"]) # Check if original exception is in message
    ...
    def test_assign_agent_load_module_fails(self, mock_load_module):
        ...
        result = assign_agent_to_task.invoke({"agent_name": "nonexistent_agent", "task": "Task"})

        self.assertEqual(result["status"], "failure")
        self.assertIsNone(result["result"])
        self.assertIn("An error occurred while executing agent nonexistent_agent", result["message"])
