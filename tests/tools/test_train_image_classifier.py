...
mock_history.history = {"loss": [0.5], "accuracy": [0.8], "val_loss": [0.6], "val_accuracy": [0.75]}
...
mock_MobileNetV2 = MagicMock(name="MobileNetV2_Class", return_value=mock_base_model_instance)
...
mock_applications.mobilenet_v2.preprocess_input = MagicMock(name="MobileNetV2_Preprocess")
...
try:
    train_image_classifier_module = importlib.import_module("tools.train_image_classifier")
    ...
    print("Error: Could not import tools.train_image_classifier. Check PYTHONPATH or test execution directory.")
...
class TestTrainImageClassifier(unittest.TestCase):
    ...
    def test_train_image_classifier_success_binary(self, mock_exists, mock_makedirs, mock_uuid):
        # Arrange
        ...
        type(mock_train_generator_instance).num_classes = PropertyMock(return_value=2) # Binary case
        ...
        mock_exists.assert_any_call(self.model_dir) # Check if exists was called for the dir
        mock_makedirs.assert_called_once_with(self.model_dir)

        # Check base model loading
        mock_MobileNetV2.assert_called_once_with(input_shape=(224, 224, 3), include_top=False, weights='imagenet')
        ...
        # Check model saving
        expected_path = os.path.join(self.model_dir, "MobileNetV2_12345678-1234-5678-1234-567812345678.h5")
        mock_model_instance.save.assert_called_once_with(expected_path)

        # Check result structure and content
        self.assertIsNone(result.get("error"), f"Expected no error, but got: {result.get('error')}")
    ...
    @patch('os.makedirs')
    @patch('os.path.exists')
    def test_train_image_classifier_success_multiclass(self, mock_exists, mock_makedirs, mock_uuid):
        # Arrange
        mock_exists.side_effect = lambda path: False if path == self.model_dir else True
        type(mock_train_generator_instance).num_classes = PropertyMock(return_value=10) # Multiclass
        ...
        mock_makedirs.assert_called_once_with(self.model_dir)
        mock_ResNet50.assert_called_once_with(input_shape=(128, 128, 3), include_top=False, weights='imagenet')
        self.assertEqual(mock_base_model_instance.trainable, False)
        ...
        expected_path = os.path.join(self.model_dir, "ResNet50_abcdef12-abcd-ef12-abcd-ef12abcdef12.h5")
        mock_model_instance.save.assert_called_once_with(expected_path)
        self.assertIsNone(result.get("error"), f"Expected no error, but got: {result.get('error')}")
        self.assertEqual(result.get("saved_model_path"), expected_path)
        self.assertEqual(result.get("training_history"), mock_history.history)
    ...
    def test_unsupported_base_model(self):
        ...
        self.assertEqual(result.get("error"), "Base model 'UnsupportedNet' not supported.")
    ...
    @patch.object(mock_model_instance, 'fit', side_effect=Exception("Test Keras Fit Error"))
    @patch('os.makedirs')
    @patch('os.path.exists')
    def test_keras_exception_handling(self, mock_exists, mock_makedirs, mock_fit_error_method):
         # Arrange
        mock_exists.side_effect = lambda path: False if path == self.model_dir else True
        type(mock_train_generator_instance).num_classes = PropertyMock(return_value=2)
        ...
        self.assertIn("An error occurred during training: Test Keras Fit Error", result.get("error"))
    ...
    def test_invalid_generator_input(self):
        ...
        # Arrange
        invalid_generator = "not a generator" # Pass a string instead of a mocked object
        ...
        self.assertIsNone(result.get("saved_model_path"))
        self.assertIsNone(result.get("training_history"))
        self.assertEqual(result.get("error"), "Invalid generator objects provided. Data generators cannot be passed directly as arguments in this environment.")
