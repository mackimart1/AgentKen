...
try:
    ...
    with warnings.catch_warnings():
        ...
        from tools.ner_tool import ner_tool as ner_tool_func, _model_loaded as ner_model_loaded_status
    ner_tool = ner_tool_func
    _TOOL_IMPORTED = True
    _MODEL_LOADED = ner_model_loaded_status
    if not _MODEL_LOADED:
        print("Warning: spaCy model 'en_core_web_sm' was not loaded during import. Tests requiring the model will be skipped.")
...
class TestNerTool(unittest.TestCase):
    ...
    @unittest.skipUnless(_MODEL_LOADED, "Skipping test: spaCy model 'en_core_web_sm' could not be loaded.")
    def test_ner_extraction(self):
        ...
        self.assertNotIn("error", result, f"Tool returned an error: {result.get('error')}")
        ...
        self.assertTrue(expected_set.issubset(result_set), f"Expected {expected_set} to be a subset of {result_set}")
