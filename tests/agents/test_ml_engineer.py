...
class TestMLEngineerAgent(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        try:
            ...
            raise unittest.SkipTest(f"Skipping tests: Failed to import agent module {cls.AGENT_MODULE_PATH}: {e}")
        except AttributeError:
            raise unittest.SkipTest(f"Skipping tests: Agent function 'ml_engineer' not found in {cls.AGENT_MODULE_PATH}")
        except Exception as e:
            raise unittest.SkipTest(f"Skipping tests: Error importing agent {cls.AGENT_MODULE_PATH}: {e}")


    def test_agent_file_exists(self):
        """Check if the agent file exists."""
        self.assertTrue(os.path.exists(self.AGENT_FILE_PATH), f"Agent file does not exist: {self.AGENT_FILE_PATH}")

    def test_agent_importable_and_callable(self):
        """Check if the agent function is callable (already checked in setUpClass)."""
        self.assertTrue(callable(self.ml_engineer), "ml_engineer function is not callable.")

    def test_basic_invocation(self):
        """Perform a basic invocation to check for runtime errors in the graph definition."""
        try:
            initial_state = {
                "messages": [("human", "Test task for basic invocation.")]
            }
            ...
            self.assertTrue(len(messages) > 1, "Agent did not produce sufficient output messages.")
            ...
            self.assertIsInstance(last_message, tuple, f"Last message is not a tuple: {last_message}")
            last_role, last_msg = last_message
            self.assertIn(last_role, ["ai", "function"], f"Unexpected role in last message: {last_role}")
            self.assertTrue(
                "Finished execution simulation." in last_msg or "error occurred" in last_msg,
            ...
            for msg in messages:
                 ...
                 content = msg[1] if isinstance(msg, tuple) and len(msg) > 1 else str(msg)
