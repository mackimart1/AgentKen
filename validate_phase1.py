...
class Phase1Validator:
    ...
    def check_python_version(self) -> bool:
        ...
        if version.major == 3 and version.minor >= 11:
            ...
            self.log_error(f"Python version: {version.major}.{version.minor}.{version.micro} (requires 3.11+)")
            return False
    ...
    def check_git_hook(self) -> bool:
        ...
        if not os.access(hook_path, os.X_OK):
            self.log_warning("Git pre-commit hook: not executable (run: chmod +x .git/hooks/pre-commit)")
            return False
