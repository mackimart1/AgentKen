...
from core.roles import (
    role_manager, Permission, UserRole, requires_permission, 
    log_audit_event, create_default_admin
)
...
class EntityCreator:
    ...
    def create_agent(self, user_id: str, agent_name: str, description: str, 
                    capabilities: List[str], author: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            # Write test file
            test_file_path = os.path.join(self.tests_dir, "agents", f"test_{agent_filename}.py")
    ...
    def create_tool(self, user_id: str, tool_name: str, description: str,
                   parameters: Dict[str, str], return_type: str, 
        ...
        try:
            ...
            # Write test file
            test_file_path = os.path.join(self.tests_dir, "tools", f"test_{tool_filename}.py")
            with open(test_file_path, 'w') as f:
                f.write(test_code)
...
def main():
    """Main CLI interface"""
    parser = argparse.ArgumentParser(description="Create agents and tools with permission enforcement")
    parser.add_argument("--user-id", required=True, help="User ID for permission checks")
    parser.add_argument("--entity-type", choices=["agent", "tool"], required=True, 
                       help="Type of entity to create")
    parser.add_argument("--name", required=True, help="Name of the entity")
    parser.add_argument("--description", required=True, help="Description of the entity")
    parser.add_argument("--author", help="Author name (defaults to user-id)")
    
    # Agent-specific arguments
    parser.add_argument("--capabilities", nargs="+", help="Agent capabilities (for agents)")
    ...
    parser.add_argument("--list-type", choices=["agents", "tools", "all"], default="all",
