...
class InferraVClient:
    ...
    def __init__(self, base_url: str = "http://localhost:5000"):
        ...
        self.session.headers.update({
            ...
            'User-Agent': 'InferraV-Client/1.0'
        })
    ...
    def monitor_plan(self, plan_id: str, timeout: int = 60) -> Dict[str, Any]:
        ...
        while time.time() - start_time < timeout:
            ...
            print(f"📊 Progress: {status['progress']:.1%} ({status['completed_tasks']}/{status['total_tasks']} tasks)")
    ...
    while True:
        try:
            ...
            if command == 'quit':
                ...
                status = client.get_status()['data']
                print(f"Health: {status['health_score']:.1f}/100, Plans: {status['total_plans']}")
            elif command == 'create':
                ...
                plan_id = client.create_plan(
                    ...
                    tasks=[{
                        ...
                        'agent_type': 'research_agent',
                        'capability': 'web_search',
                        'parameters': {'query': 'interactive test', 'max_results': 2},
                        'priority': 2,
                        'estimated_duration': 3.0,
                        'dependencies': [],
                        'required_resources': ['internet_access']
